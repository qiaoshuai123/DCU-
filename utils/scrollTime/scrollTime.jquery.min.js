            ...
            if ($(e.currentTarget).attr('class')) {
              timeInter = setInterval(() => {
                ...
                _thisDom.setState({
                  ...
                  const timeNumList = $($("#timeBox").find(`.${this.plugStyle.horStepGapBig}`))
                    if ($(timeNumList[_thisDom.state.nowCycleLength ? _thisDom.state.nowCycleLength-1 : _thisDom.state.nowCycleLength]).attr('style').indexOf('green') > -1) {
                      _thisDom.setState({
                        lightStatus: 5,
                      })
                    } else if ($(timeNumList[_thisDom.state.nowCycleLength ? _thisDom.state.nowCycleLength-1 : _thisDom.state.nowCycleLength]).attr('style').indexOf('yellow') > -1) {
                      _thisDom.setState({
                        lightStatus: 8,
                      })
                    } else {
